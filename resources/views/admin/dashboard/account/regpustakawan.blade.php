            ...
            <div class="col-xs-12">
              <div class="box">
                <div class="box-header">
                  <h3 class="box-title">Daftar Akun Pustakawan</h3>
                </div><!-- /.box-header -->
                ...
                <div class="box-body">
                  <table id="dataKurikulum" class="table table-bordered table-hover">
                    ...
                    <tbody>
                     <?php foreach ($listAccountPustakawan as $itemAccount):  ?>
                      <tr>
                        <td>{{$itemAccount->pusNip}}</td>
                        <td>{{$itemAccount->pusNama}}</td>
                        <td>{{$itemAccount->pusEmail}}</td>
                        ...
                        <td>
                            ...
                            <a class="btn btn-success btn-flat btn-sm" onClick="tampilModal('{{$itemAccount->pusNip}}','{{$itemAccount->pusNama}}','{{$itemAccount->pusEmail}}')" id="registerPustakawan{{{$itemAccount->pusNip}}}" title="Register"><i class="fa fa-plus"></i></a>
                            @else
                           
                            <a class="btn btn-danger btn-flat btn-sm" href="{{{ URL::to('accountpustakawan/'.$itemAccount->id.'/hapus') }}}" title="Hapus Akun"><i class="fa fa-trash"> Unreg </i></span>
                            </a> 
                            @endif
              ...
              <div class="modal-dialog">
                  <div class="modal-content">
                      <div class="modal-header">
                          ...
                          <h4 class="modal-title" id="myModalLabel">Register Pustakawan - Tambah</h4>
                      </div>
                      <div class="modal-body">
           
                          <form id="formRegisterPustakawan" class="form-horizontal" role="form" method="POST" action="{{ url('/accountpustakawan/tambah') }}">
...
@section('script')
    ...
    <script type="text/javascript">
      ...
      $(document).on('submit', '#formRegisterPustakawan', function(e) {  
            ...
            .done(function(data) {
                ...
                window.location.href=url+'/accountpustakawan'; 
            })
            .fail(function(data) {
                console.log(data.responeJSON);
                $.each(data.responseJSON, function (key, value) {
                    var input = '#formRegisterPustakawan input[name=' + key + ']';
